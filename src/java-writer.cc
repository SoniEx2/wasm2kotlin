...
typedef Name<3> ExternalReadRef;
typedef Name<4> ExternalWriteRef;
...
class JavaWriter {
 public:
  JavaWriter(Stream* java_stream,
          const char* class_name,
          const char* package_name,
          const WriteJavaOptions& options)
      : options_(options),
        java_stream_(java_stream),
 ...
 private:
  ...
  std::string ExternalRefOf(const std::string&);
  std::string TypeNameOf(const std::string&);
  std::string AddressOf(const std::string&, const std::string&);
  std::string DerefRead(const std::string&);
  std::string DerefWrite(const std::string&);
  ...
  void Write(const ExternalReadRef&);
  void Write(const ExternalWriteRef&);
  void Write(Type);
  ...
  void WriteFuncTypes();
  void WriteImports();
  void WriteFuncImport(const std::string&, const std::string&);
  ...
  const WriteJavaOptions& options_;
  const Module* module_ = nullptr;
  const Func* func_ = nullptr;
  Stream* stream_ = nullptr;
  MemoryStream funjava_stream_;
  Stream* java_stream_ = nullptr;
...
#include "src/prebuilt/wasm2java.include.java"
#undef SECTION_NAME

size_t JavaWriter::MarkTypeStack() const {
  return type_stack_.size();
}

void JavaWriter::ResetTypeStack(size_t mark) {
  assert(mark <= type_stack_.size());
  type_stack_.erase(type_stack_.begin() + mark, type_stack_.end());
}

Type JavaWriter::StackType(Index index) const {
  assert(index < type_stack_.size());
  return *(type_stack_.rbegin() + index);
}

void JavaWriter::PushType(Type type) {
  type_stack_.push_back(type);
}

void JavaWriter::PushTypes(const TypeVector& types) {
  type_stack_.insert(type_stack_.end(), types.begin(), types.end());
}

void JavaWriter::DropTypes(size_t count) {
  assert(count <= type_stack_.size());
  type_stack_.erase(type_stack_.end() - count, type_stack_.end());
}

void JavaWriter::PushLabel(LabelType label_type,
                        const std::string& name,
...
const Label* JavaWriter::FindLabel(const Var& var) {
  Label* label = nullptr;
...
void JavaWriter::PopLabel() {
  label_stack_.pop_back();
}

std::string JavaWriter::ExternalRefOf(const std::string& name) {
  if (type_map_.count(name) == 1) {
    assert(extkind_map_.count(name) == 0);
    auto iter = type_map_.find(name);
    assert(iter != type_map_.end());
    Type t = iter->second;
    switch (t) {
      case Type::I32: return "wasm_rt.Impl.ExternalRefInt";
      case Type::I64: return "wasm_rt.Impl.ExternalRefLong";
      case Type::F32: return "wasm_rt.Impl.ExternalRefFloat";
      case Type::F64: return "wasm_rt.Impl.ExternalRefDouble";
      case Type::Func: return "wasm_rt.Impl.ExternalRef<java.lang.invoke.MethodHandle>";
      default: assert(false && "Unknown Type");
    }
  } else {
    assert(type_map_.count(name) == 0);
    assert(extkind_map_.count(name) == 1);
    auto iter = extkind_map_.find(name);
    assert(iter != extkind_map_.end());
    ExternalKind t = iter->second;
    switch (t) {
      case ExternalKind::Memory: return "wasm_rt.Impl.ExternalRef<wasm_rt.Impl.Memory>";
      case ExternalKind::Table: return "wasm_rt.Impl.ExternalRef<wasm_rt.Impl.Table>";
      default: assert(false && "Unknown ExternalKind");
    }
  }
}

std::string JavaWriter::TypeNameOf(const std::string& name) {
  if (type_map_.count(name) == 1) {
    assert(extkind_map_.count(name) == 0);
    auto iter = type_map_.find(name);
    assert(iter != type_map_.end());
    ...
    switch (t) {
      ...
      case Type::Func: return "java.lang.invoke.MethodHandle";
      default: assert(0);
    }
  } else {
    assert(type_map_.count(name) == 0);
    ...
    ExternalKind t = iter->second;
    switch (t) {
      case ExternalKind::Memory: return "wasm_rt.Impl.Memory";
      case ExternalKind::Table: return "wasm_rt.Impl.Table";
      case ExternalKind::Func: return "0";
      default: assert(0);
    }
  }
}
...
// static
std::string JavaWriter::AddressOf(const std::string& s, const std::string& class_name) {
  std::string ext = ExternalRefOf(s);
  std::string st = TypeNameOf(s);
  return "(new " + ext + "(){public " + st + " get(){return " + class_name + ".this." + s + ";}public void set(" + st + " value){" + class_name + ".this." + s + "=value;}})";
}

// static
std::string JavaWriter::DerefRead(const std::string& s) {
  return "(" + s + "get())";
}

// static
std::string JavaWriter::DerefWrite(const std::string& s) {
  return s + ".set(";
}

// static
char JavaWriter::MangleType(Type type) {
  switch (type) {
...
// static
std::string JavaWriter::MangleTypes(const TypeVector& types) {
  if (types.empty())
    return std::string("v");
...
// static
std::string JavaWriter::MangleName(string_view name) {
...
// static
std::string JavaWriter::MangleFuncName(string_view name,
                                    const TypeVector& param_types,
                                    const TypeVector& result_types) {
...
// static
std::string JavaWriter::MangleGlobalName(string_view name, Type type) {
  std::string sig(1, MangleType(type));
  return MangleName(name) + MangleName(sig);
}

// static
std::string JavaWriter::ExportName(string_view mangled_name) {
  return mangled_name.to_string();
}

// static
std::string JavaWriter::LegalizeName(string_view name) {
  if (name.empty())
    return "__";
...
std::string JavaWriter::DefineName(SymbolSet* set, string_view name) {
...
std::string JavaWriter::DefineImportName(const std::string& name,
                                      string_view module,
                                      string_view mangled_field_name,
				      Type type) {
...
std::string JavaWriter::DefineGlobalScopeName(const std::string& name, Type type) {
...
std::string JavaWriter::DefineGlobalScopeName(const std::string& name, ExternalKind type) {
  std::string unique = DefineName(&global_syms_, StripLeadingDollar(name));
  global_sym_map_.insert(SymbolMap::value_type(name, unique));
...
void JavaWriter::Indent(int size) {
  indent_ += size;
}

void JavaWriter::Dedent(int size) {
  indent_ -= size;
  assert(indent_ >= 0);
}

void JavaWriter::WriteIndent() {
...
void JavaWriter::WriteData(const void* src, size_t size) {
...
void WABT_PRINTF_FORMAT(2, 3) JavaWriter::Writef(const char* format, ...) {
  WABT_SNPRINTF_ALLOCA(buffer, length, format);
  WriteData(buffer, length);
}

void JavaWriter::Write(Newline) {
  Write("\n");
  should_write_indent_next_ = true;
}

void JavaWriter::Write(OpenBrace) {
  Write("{");
  Indent();
  Write(Newline());
}

void JavaWriter::Write(CloseBrace) {
  Dedent();
  Write("}");
}

void JavaWriter::Write(Index index) {
  Writef("%" PRIindex, index);
}

void JavaWriter::Write(string_view s) {
  WriteData(s.data(), s.size());
}

void JavaWriter::Write(const LocalName& name) {
  assert(local_sym_map_.count(name.name) == 1);
  Write(local_sym_map_[name.name]);
}

std::string JavaWriter::GetGlobalName(const std::string& name) const {
...
//Type JavaWriter::TypeOf(const std::string& name) const {
...
void JavaWriter::Write(const GlobalName& name) {
  Write(GetGlobalName(name.name));
}

void JavaWriter::Write(const ExternalPtr& name) {
...
void JavaWriter::Write(const ExternalReadRef& name) {
  bool is_import = import_syms_.count(name.name) != 0;
  if (is_import) {
    Write(DerefRead(GetGlobalName(name.name)));
  } else {
    Write(GetGlobalName(name.name));
  }
}

void JavaWriter::Write(const ExternalWriteRef& name) {
  bool is_import = import_syms_.count(name.name) != 0;
  if (is_import) {
    Write(DerefWrite(GetGlobalName(name.name)));
  } else {
    Write(GetGlobalName(name.name));
  }
}

void JavaWriter::Write(const Var& var) {
  assert(var.is_name());
  Write(LocalName(var.name()));
}

void JavaWriter::Write(const GotoLabel& goto_label) {
...
void JavaWriter::Write(const LabelDecl& label) {
  Write(label.name, ":", Newline());
}

void JavaWriter::Write(const GlobalVar& var) {
  assert(var.var.is_name());
  Write(ExternalReadRef(var.var.name()));
}

void JavaWriter::Write(const StackVar& sv) {
...
void JavaWriter::Write(Type type) {
  switch (type) {
...
void JavaWriter::Write(TypeEnum type) {
  switch (type.type) {
    case Type::I32: Write("Integer.class"); break;
...
void JavaWriter::Write(SignedType type) {
  switch (type.type) {
    case Type::I32: Write("int"); break;
    case Type::I64: Write("long"); break;
    default:
...
void JavaWriter::Write(const ResultType& rt) {
  if (!rt.types.empty()) {
    Write(rt.types[0]);
  } else {
    Write("void");
...
void JavaWriter::Write(const Const& const_) {
  switch (const_.type()) {
    case Type::I32:
...
void JavaWriter::WriteInitExpr(const ExprList& expr_list) {
  if (expr_list.empty())
    return;
...
void JavaWriter::WriteSourceTop() {
  if (!package_name_.empty()) {
    Write("package ", package_name_, ";", Newline());
  }
  WriteImports();
  Write(s_source_includes);
  Write("public class ", class_name_, " {");
  Indent();
}

void JavaWriter::WriteSourceBottom() {
  Dedent();
  Write("}");
}

void JavaWriter::WriteFuncTypes() {
  Write(Newline());
  Writef("private static int func_types[] = new int[%" PRIzd "];", module_->types.size());
  Write(Newline(), Newline());
  Write("private static void init_func_types() {", Newline());
  ...
  for (TypeEntry* type : module_->types) {
    ...
    Write("  func_types[", func_type_index, "] = wasm_rt.Impl.register_func_type(",
    ...
    switch (import->kind()) {
	    ...
	    import->module_name,
            DefineImportName(
                ...
                MangleFuncName(import->field_name, func.decl.sig.param_types,
                               func.decl.sig.result_types), ExternalKind::Func));
        Write(";");
        break;
      }
      ...
      case ExternalKind::Global: {
        ...
        WriteGlobal(global,
                    DefineImportName(
                        ...
                        MangleGlobalName(import->field_name, global.type), global.type));
        Write(";");
        break;
      }
      ...
      case ExternalKind::Memory: {
        ...
        WriteMemory(DefineImportName(memory.name, import->module_name,
                                     MangleName(import->field_name), ExternalKind::Memory));
        break;
      }
      ...
      case ExternalKind::Table: {
        ...
        WriteTable(DefineImportName(table.name, import->module_name,
                                    MangleName(import->field_name), ExternalKind::Table));
        break;
      }
...
void JavaWriter::WriteFuncDeclaration(const FuncDeclaration& decl,
                                   const std::string& name) {
  Write("wasm_rt.Impl.ExternalRef<java.lang.invoke.MethodHandle> ", name);
}

void JavaWriter::WriteFuncDeclarations() {
  ...
  for (const Func* func : module_->funcs) {
    ...
    if (!is_import) {
      Write("private java.lang.invoke.MethodHandle ", DefineGlobalScopeName(func->name, Type::Func));
      Write(";", Newline());
    }
    ++func_index;
  }
...
void JavaWriter::WriteFuncImport(const std::string& module_name,
                                   const std::string& name) {
  if (!package_name_.empty()) {
    Write(package_name_, ".");
  }
  Write(module_name, ".", name);
  Write(";", Newline());
}

void JavaWriter::WriteGlobals() {
  ...
  Write(Newline(), "private void init_globals() ", OpenBrace());
  global_index = 0;
  for (const Global* global : module_->globals) {
    bool is_import = global_index < module_->num_global_imports;
    if (!is_import) {
...
void JavaWriter::WriteGlobal(const Global& global, const std::string& name) {
  Write(global.type, " ", name);
}

void JavaWriter::WriteMemories() {
...
void JavaWriter::WriteMemory(const std::string& name) {
  Write("wasm_rt.Impl.Memory ", name, ";");
}

void JavaWriter::WriteTables() {
...
void JavaWriter::WriteTable(const std::string& name) {
  Write("wasm_rt.Impl.Table ", name, ";");
}

void JavaWriter::WriteDataInitializers() {
  ...
  if (!module_->memories.empty()) {
    if (module_->data_segments.empty()) {
      ...
      for (const DataSegment* data_segment : module_->data_segments) {
        Write(Newline(), "private static byte[] data_segment_data_",
  ...
  Write(Newline(), "private void init_memory() ", OpenBrace());
  if (memory && module_->num_memory_imports == 0) {
    uint32_t max =
        memory->page_limits.has_max ? memory->page_limits.max : 65536;
    Write("wasm_rt.Impl.allocate_memory(", ExternalPtr(memory->name), ", ",
  ...
  for (const DataSegment* data_segment : module_->data_segments) {
    Write(ExternalReadRef(memory->name), ".put(");
...
void JavaWriter::WriteInitExports() {
  Write(Newline(), "private void init_exports() ", OpenBrace());
  WriteExports(WriteExportsKind::Initializers);
  Write(CloseBrace(), Newline());
}

void JavaWriter::WriteExports(WriteExportsKind kind) {
  ...
  for (const Export* export_ : module_->exports) {
    ...
    switch (export_->kind) {
      ...
      case ExternalKind::Memory: {
        ...
        if (kind != WriteExportsKind::Initializers) {
          Write(ExternalRefOf(mangled_name), mangled_name, ";");
        }
        break;
      }
...
void JavaWriter::WriteInit() {
  Write(Newline(), "public ", class_name_, "() ", OpenBrace());
  ...
  for (Var* var : module_->starts) {
    Write(ExternalReadRef(module_->GetFunc(*var)->name), "();", Newline());
  }
  Write(CloseBrace(), Newline());
}

void JavaWriter::WriteFuncs() {
  Index func_index = 0;
  for (const Func* func : module_->funcs) {
    bool is_import = func_index < module_->num_func_imports;
    if (!is_import)
...
void JavaWriter::Write(const Func& func) {
  func_ = &func;
  ...
  Write("private static ", ResultType(func.decl.sig.result_types), " ",
        GlobalName(func.name), "(");
  WriteParamsAndLocals();
  //Write("FUNC_PROLOGUE;", Newline());

  stream_ = &funjava_stream_;
  stream_->ClearOffset();
  ...
  stream_ = java_stream_;

  WriteStackVarDeclarations();

  std::unique_ptr<OutputBuffer> buf = funjava_stream_.ReleaseOutputBuffer();
  stream_->WriteData(buf->data.data(), buf->data.size());

  Write(CloseBrace());

  funjava_stream_.Clear();
  func_ = nullptr;
}

void JavaWriter::WriteParamsAndLocals() {
...
void JavaWriter::WriteParams(const std::vector<std::string>& index_to_name) {
...
void JavaWriter::WriteLocals(const std::vector<std::string>& index_to_name) {
...
void JavaWriter::WriteStackVarDeclarations() {
  for (Type type : {Type::I32, Type::I64, Type::F32, Type::F64}) {
    size_t count = 0;
...
void JavaWriter::Write(const ExprList& exprs) {
  for (const Expr& expr : exprs) {
	...
	Write(label, ": ");
      ...
      case ExprType::MemorySize: {
        ...
        PushType(Type::I32);
        Write(StackVar(0), " = ", ExternalReadRef(memory->name), ".pages;",
              Newline());
        break;
      }
...
void JavaWriter::WriteSimpleUnaryExpr(Opcode opcode, const char* op) {
...
void JavaWriter::WriteInfixBinaryExpr(Opcode opcode,
                                   const char* op,
                                   AssignOp assign_op,
				   bool debooleanize) {
  Type result_type = opcode.GetResultType();
...
void JavaWriter::WritePrefixBinaryExpr(Opcode opcode, const char* op) {
  Type result_type = opcode.GetResultType();
...
void JavaWriter::WriteUnsignedCompareExpr(Opcode opcode, const char* op) {
  Type result_type = opcode.GetResultType();
...
void JavaWriter::Write(const BinaryExpr& expr) {
  switch (expr.opcode) {
    case Opcode::I32Add:
    case Opcode::I64Add:
    case Opcode::F32Add:
...
void JavaWriter::Write(const CompareExpr& expr) {
  switch (expr.opcode) {
    case Opcode::I32Eq:
    case Opcode::I64Eq:
    case Opcode::F32Eq:
...
void JavaWriter::Write(const ConvertExpr& expr) {
  switch (expr.opcode) {
    case Opcode::I32Eqz:
    case Opcode::I64Eqz: {
    ...
    case Opcode::I32TruncF32S:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.I32_TRUNC_S_F32");
      break;

    case Opcode::I64TruncF32S:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.I64_TRUNC_S_F32");
      break;

    case Opcode::I32TruncF64S:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.I32_TRUNC_S_F64");
      break;

    case Opcode::I64TruncF64S:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.I64_TRUNC_S_F64");
      break;

    case Opcode::I32TruncF32U:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.I32_TRUNC_U_F32");
      break;

    case Opcode::I64TruncF32U:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.I64_TRUNC_U_F32");
      break;

    case Opcode::I32TruncF64U:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.I32_TRUNC_U_F64");
      break;

    case Opcode::I64TruncF64U:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.I64_TRUNC_U_F64");
      break;
    ...
    case Opcode::F32ConvertI32S:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.F32_CVT_S_I32");
      break;

    case Opcode::F32ConvertI64S:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.F32_CVT_S_I64");
      break;

    case Opcode::F32ConvertI32U:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.F32_CVT_U_I32");
      break;
    ...
    case Opcode::F32ConvertI64U:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.F32_CVT_U_I64");
      break;

    case Opcode::F64ConvertI32S:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.F64_CVT_S_I32");
      break;

    case Opcode::F64ConvertI64S:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.F64_CVT_S_I64");
      break;

    case Opcode::F64ConvertI32U:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.F64_CVT_U_I32");
      break;
    ...
    case Opcode::F64ConvertI64U:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.F64_CVT_U_I64");
      break;

    case Opcode::F32ReinterpretI32:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.f32_reinterpret_i32");
      break;

    case Opcode::I32ReinterpretF32:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.i32_reinterpret_f32");
      break;

    case Opcode::F64ReinterpretI64:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.f64_reinterpret_i64");
      break;

    case Opcode::I64ReinterpretF64:
      WriteSimpleUnaryExpr(expr.opcode, "wasm_rt.Impl.i64_reinterpret_f64");
      break;
...
void JavaWriter::Write(const LoadExpr& expr) {
...
void JavaWriter::Write(const StoreExpr& expr) {
  const char* func = nullptr;
  switch (expr.opcode) {
...
void JavaWriter::Write(const UnaryExpr& expr) {
  switch (expr.opcode) {
    case Opcode::I32Clz:
...
void JavaWriter::Write(const TernaryExpr& expr) {
  switch (expr.opcode) {
...
void JavaWriter::Write(const SimdLaneOpExpr& expr) {
  Type result_type = expr.opcode.GetResultType();
...
void JavaWriter::Write(const SimdShuffleOpExpr& expr) {
  Type result_type = expr.opcode.GetResultType();
  Write(StackVar(1, result_type), " = ", expr.opcode.GetName(), "(",
...
void JavaWriter::Write(const LoadSplatExpr& expr) {
  assert(module_->memories.size() == 1);
  Memory* memory = module_->memories[0];
...
void JavaWriter::WriteJavaSource() {
  stream_ = java_stream_;
  WriteSourceTop();
...
Result JavaWriter::WriteModule(const Module& module) {
  WABT_USE(options_);
  module_ = &module;
  WriteJavaSource();
  return result_;
...
Result WriteJava(Stream* java_stream,
              const char* class_name,
              const char* package_name,
              const Module* module,
              const WriteJavaOptions& options) {
  JavaWriter java_writer(java_stream, class_name, package_name, options);
  return java_writer.WriteModule(*module);
}
